...
def graph_answering(graph_with_knowledge,question,model):
    ...
    qa_prompt = "Please judge whether the following statements can be substantiated or not based on your own knowledge and the relevant knowledge given, if they can be substantiated please output 'Supported', otherwise output 'Refuted'.\n\nThe relevant knowledge given is:\n{0}\n\nThe statement is: {1}".format("\n".join(reasoning_path),question)
...
def load_dataset(file_path):
    ...
    return question[:500], gold_answer[:500]
...
if __name__ == "__main__":
    model = load_model()
    question,gold_answer = load_dataset("datasets/fever-dev.jsonl")
    answer = testing(question,model)
    evaluation_and_save(question,answer,gold_answer,"output_fever.json",model) 